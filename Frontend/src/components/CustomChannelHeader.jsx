...
import PinnedMessagesModal from "./MembersModal"; // Assuming PinnedMessagesModal is in the same folder
...
const CustomChannelHeader = () => {
	...
	const handleVideoCall = async () => {
		if (channel) {
			// 🏆 CRITICAL FIX: Ensure only the clean ID is passed to the URL.
			// The video client ("messaging", callId) already specifies the type.
			// We strip the type prefix (e.g., "messaging:general" -> "general").
			const cleanId = channel.id.split(':').pop();

			const callUrl = `${window.location.origin}/call/${cleanId}`;
